...
chatconfig.cfg - bpqchat server configuration.
		 ...
		 A bit off topic but important to know:
                 ...
                 Once linked into the network, you should be able to reach either RRRCHT or RRR001
                 depending on which UDP node you are linked to. No matter which one you can reach
                 there still needs to be some manual configuration done on them to allow your node
                 to join the network, so if intending to join please get in touch with your node
                 and chat server's details. pe1rrr@amsat.org
...
============================================================================================================
	...
	gdbwait - GDB debugging script which will automatically attach to the running linbpq process.
...
The gdbwait script is provided so that you can monitor the process while it is running. If it crashes
